...
def get_mutual(
    ...
    if target_uid:
        ...
        for i in range(tries):
            try:
                url = f"friends.getMutual?access_token={access_token}&source_uid={source_uid}&target_uid={target_uid}&target_uids={','.join(list(map(str, target_uids)))}&count={count}&offset={i*100}&v={v}"  # type: ignore
                response = s.get(url)
